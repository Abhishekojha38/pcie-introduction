...
* A `Root Port` is a port on the Root Complex where another PCIe Endpoint (aka a physical ‘device’) or Switch (aka a ‘router’) can be connected to
...
* A `PCI Express Switch` is a device that routes packets to multiple ports and therefore multiple Endpoints at once.

* A `Endpoint(EP)` is a device that can be connected to a Root Port or Switch Port.

* A `PCI Express Port(root port)` is a logical PCI-PCI Bridge structure.
