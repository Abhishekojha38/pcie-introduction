...
Lets start with bookish definition:
...
I am not going to cover the history of PCIe in this document. 

* PCIe was brought to replace PCI with followig improvements. PCIe uses serial bus technologies where as PCI is parallel bus so PCIe uses less IO pins which reduces the layout complexity thus reduce the board design cost.
...
We will cover two usecases here:
