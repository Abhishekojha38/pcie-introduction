...
PCIe stands for Peripheral Component Interconnect Express. It is a motherboard expansion bus standard introduced in 2003 to enable high-speed serial communication between the CPU and its peripheral components.

image::images/Timeline of PCIe Interface.png[]

* PCI express design to replace PCI by making sure that PCIe software support backword compatibility.

* PCIe Improvements:

** PCIe uses serial bus technologies where as PCI is parallel bus so PCIe uses less IO pins which reduces the layout complexity thus reduce the board design cost.

** Allows to increase the transmission frequency.
...
** Allow to scale bandwidth by providing support for multiple lane and that will increase the throughput.1-32 lanes support
...
* CPU will also use Root complex in order to access the endpoint devices.
...
* It also provide the address to which data can read or written.
...
[NOTE]
--
PCI address space is visible to the Only PCIe components like Root complex, endpoints, Switches and bridges etc.
--
...
| PCI Express Port(root port)
| `PCI Express Port(root port)` is a logical PCI-PCI Bridge structure.
